...
def plot_arc(plot_file, seq, pairs, start_pos=1, height_ratio=None, height_margin=0.05, title=None, dpi=300):
    """
...
def draw_structure(seq, pairs):
    ...
    class Nucleotide(object):
        def __init__(self, number, base, partner, coords):
            self.number = number
            self.base = base
            self.partner = partner
            self.coords = coords
            self.is_paired = partner != 0
            self.paired_to = np.asarray([
                num == self.partner for num, base in enumerate(seq, start=1)
            ])
            self.bonded_to = np.asarray([
                abs(num - self.number) == 1 for num, base in enumerate(seq, start=1)
            ])
            self.away_from = np.logical_not(np.logical_or(self.paired_to, self.bonded_to))
            self.away_from[self.number - 1] = 0
        def paired_energy(self, pair_dists):
            return np.sum((2.0 - pair_dists[self.paired_to])**2)
        def bonded_energy(self, pair_dists):
            return np.sum((1.0 - pair_dists[self.bonded_to])**2)
        def away_energy(self, pair_dists):
            return np.sum(pair_dists[self.away_from]**-2)
        def get_energy(self, pair_dists):
            dists = pair_dists[self.number - 1]
            energy = self.paired_energy(dists) + self.bonded_energy(dists) + self.away_energy(dists)
            return energy
    def get_energy(coords, nucleotides):
        coords = coords.reshape((n_bases, 2))
        pair_dists = squareform(pdist(coords))
        energy = sum([nt.get_energy(pair_dists) for num, nt in nucleotides.items()])
        if abs(round(energy) - energy) < 0.1:
            print(energy)
        return energy
    partners = {num: 0 for num, base in enumerate(seq, start=1)}
    for num1, num2 in pairs:
        assert num1 <= n_bases
        assert num2 <= n_bases
        partner1 = partners[num1]
        if partner1 != 0 and partner1 != num2:
            print(num1, num2, partner1)
            raise ValueError("Invalid pair")
        partner2 = partners[num2]
        if partner2 != 0 and partner2 != num1:
            raise ValueError("Invalid pair")
        partners[num1] = num2
        partners[num2] = num1
    coords = np.vstack([
        [float(num) for num in range(len(seq))],
        [0.0 for num in range(len(seq))]
    ]).reshape(n_bases * 2, order="F")
    nucleotides = {num:
        Nucleotide(num, base, partners[num], coords)
        for num, base in enumerate(seq, start=1)
    }
    result_opt = minimize(get_energy, coords, args=(nucleotides,), tol=1.0)
    coords_opt = result_opt.x.reshape((n_bases, 2))
    for idx, base in enumerate(seq):
        num = idx + 1
        x, y = coords_opt[idx]
        plt.text(x, y, base)
    plt.scatter(coords_opt[:, 0], coords_opt[:, 1], s=0.0)
    plt.savefig("5utr_coords_1.png")
    plt.close()


if __name__ == "__main__":
    ct_file = "5utr_test.ct"
    name, pairs, paired, seq = read_ct_file_single(ct_file)
